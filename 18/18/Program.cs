...
var sr = new StreamReader("/Users/nathanedwards/Dev/AoC/18/18/input.txt");
string[] strArr = sr.ReadToEnd().Split("\n");

// I will be reusing my code from day 16 to implement dijkstra's algorithm. I will also be formatting the 
// map to have # around all walls like in day 16

char[,] buildMap(string[] bytes, int maxLen)
{
	char[,] map = new char[73,73]; 
	...
	{
		string line = strArr[n];
		int x = Int32.Parse(line.Split(",")[0]) + 1;
		int y = Int32.Parse(line.Split(",")[1]) + 1;
		map[x, y] = '#';
	}
...
(List<int[]> nodes, _, List<long> scores) = dijkstra(map, ([],[]));
...
/* --------------- Part 2 --------------- */
// Do interval bisection starating with (1024, len(input)) to find the byte that makes the end unreachable
...
{
	int m = (n + max) / 2;
	map = buildMap(strArr, m);
	(nodes, _, scores) = dijkstra(map, ([],[]));
	endNodeIdx = findNodeIdx(nodes, endNode);
...
Console.WriteLine("Solution 2 = " + strArr[n] + " , m = " + n);
